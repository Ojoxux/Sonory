...
export class PinRepository {
	...
	private toDomainModel(record: SoundPinRecord): SoundPinAPI {
		...
		return {
			...
			weather: record.weather_temperature
				? {
						temperature: record.weather_temperature,
						condition: record.weather_condition ?? undefined,
						windSpeed: record.weather_wind_speed ?? undefined,
						humidity: record.weather_humidity ?? undefined,
					}
				: undefined,
			timeTag: record.time_tag ?? undefined,
			aiAnalysis: record.ai_transcription
				? {
						...
						summary: record.ai_summary ?? undefined,
					}
				: undefined,
			status: record.status,
			title: record.title ?? undefined,
			metadata: {
				deviceInfo: record.device_info ?? undefined,
			},
	...
	async update(id: string, data: SoundPinUpdate): Promise<SoundPinAPI | null> {
		try {
			...
			this.logger.info("Pin updated", { pinId: id, requestId: this.requestId });
			return this.toDomainModel(record);
		} catch (error) {
