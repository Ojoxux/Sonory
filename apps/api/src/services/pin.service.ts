...
import type { PostGISPoint, SoundPinInsert, SoundPinUpdate } from '../types/database'
...
export class PinService extends BaseService {
    ...
    */
   async updatePin(id: string, request: UpdatePinRequest): Promise<SoundPinAPI | null> {
   ...
   private validateAudioMetadata(audio: CreatePinRequest['audio']): void {
      if (!audio.url || audio.url.trim() === '') {
         throw new APIException(ERROR_CODES.INVALID_AUDIO_FORMAT, 'Audio URL is required', 400)
      }
   ...
   private validateStatus(status: string): void {
      const validStatuses = ['active', 'processing', 'deleted', 'reported'] as const
   ...
   private validateAIAnalysis(aiAnalysis: UpdatePinRequest['aiAnalysis']): void {
      ...
      if (!aiAnalysis.transcription || aiAnalysis.transcription.trim() === '') {
         throw new APIException(ERROR_CODES.AI_ANALYSIS_FAILED, 'AI transcription is required', 400)
      }

      if (aiAnalysis.categories.confidence < 0 || aiAnalysis.categories.confidence > 1) {
         throw new APIException(
      ...
      return {
         ...
         ...(request.aiAnalysis
            ? {
                 ...
                 ...(request.aiAnalysis.summary ? { ai_summary: request.aiAnalysis.summary } : {}),
              }
            : {}),
      }
   }
