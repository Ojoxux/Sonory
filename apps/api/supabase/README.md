...
```
supabase/
  schema.sql      実DBの完全なスナップショット（生成物・手で編集しない）
...
| `schema.sql`  | **今どうなっているか**。適用結果のスナップショット | 生成する |

食い違いが出たら、**正しいのは実DB**であり `schema.sql` の再生成が必要という合図。
再生成して `git diff` を取ると、SQL Editor での直接変更などによる乖離が
...
2. 適用したら `schema.sql` を再生成してコミットする。
