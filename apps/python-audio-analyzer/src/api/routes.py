...
async def analyze_audio_url(
    request: AudioAnalysisRequest, analyzer: AudioAnalyzer = Depends(get_audio_analyzer)
) -> AnalysisResult:
    """
    URLから音声を取得して分析
    ...
    try:
        ...
        logger.warning(
            "Invalid request parameters", request_id=request_id, error=str(e)
        )
...
async def analyze_audio_upload(
    ...
    try:
        ...
        if len(audio_bytes) > MAX_FILE_SIZE:
            raise ValueError(
                f"File too large: {len(audio_bytes)} bytes (max: {MAX_FILE_SIZE})"
            )
        ...
        result = await analyzer.analyze_audio_from_bytes(
            audio_bytes=audio_bytes, filename_hint=file.filename, top_k=top_k
        )
        ...
        logger.warning("Invalid upload request", request_id=request_id, error=str(e))
...
async def health_check(
    request: Request, analyzer: AudioAnalyzer = Depends(get_audio_analyzer)
) -> HealthResponse:
    """
    サービスのヘルスチェック
...
@router.get(
    "/stats", summary="分析統計取得", description="音響分析の統計情報を取得します"
)
