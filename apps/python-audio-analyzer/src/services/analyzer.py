...
class AudioAnalyzer:
    ...
    async def analyze_audio_from_url(
        self, audio_url: str, top_k: int = 5, max_retries: int = 3
    ) -> AnalysisResult:
        """
        URLから音声を取得して分析
    ...
    async def analyze_audio_from_file(
        ...
        logger.info(
            "Starting audio analysis from file", file_path=str(file_path), top_k=top_k
        )
        ...
        try:
            # 音声を前処理
            processed_audio = await self.audio_processor.process_audio_from_file(
                file_path
            )
            ...
            self._analyzer_stats["failed_analyses"] += 1
            logger.error(
                "Audio analysis from file failed",
                file_path=str(file_path),
                error=str(e),
            )
            raise RuntimeError(f"Audio analysis failed: {e}")
    ...
    async def _perform_analysis(
        ...
        try:
            ...
            performance_metrics = {
                ...
                "processing_ratio": yamnet_time
                / processed_audio.original_metadata.duration,
            }
    ...
    def _generate_environment_description(
        ...
        significant_others = [
            env
            for env, score in env_scores.items()
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            # YAMNetマネージャーの状態確認
            self.yamnet_manager.get_classifier()
