...
def setup_tensorflow_hub_cache():
    ...
    try:
        ...
        if not os.path.exists(python_analyzer_root):
            raise ValueError(
                f"Invalid python analyzer root path: {python_analyzer_root}"
            )
        ...
        if not cache_dir.startswith(python_analyzer_root):
            raise ValueError(
                "Invalid cache directory path - potential path traversal attack"
            )
...
class YAMNetClassifier:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            model = await loop.run_in_executor(None, self._load_model_with_retry)
            self.model = model
    ...
    def _load_model_with_retry(self) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"Attempting to load YAMNet model (attempt {attempt + 1}/{max_retries})"
                )
    ...
    def classify_audio(
        ...
        if sample_rate != 16000:
            logger.warning(
                "Sample rate is not 16kHz, may affect accuracy", sample_rate=sample_rate
            )
        ...
        try:
            ...
            logger.info("Audio waveform analysis", audio_stats=audio_stats)
            ...
            logger.info("YAMNet inference statistics", score_stats=score_stats)
    ...
    def _convert_to_japanese(
        ...
        for class_name, score in zip(class_names, scores):
            ...
            if japanese_category:
                # 同じカテゴリの最高スコアを保持
                if (
                    japanese_category not in japanese_categories
                    or score > japanese_categories[japanese_category]
                ):
                    japanese_categories[japanese_category] = float(score)
            else:
                # マッピングされないクラスも記録
                unmapped_classes.append({"class": class_name, "score": float(score)})
        ...
        sorted_results = sorted(
            normalized_categories.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _find_japanese_category(self, audioset_class: str) -> Optional[str]:
        ...
        for audioset_key, japanese_category in self.AUDIOSET_TO_JAPANESE.items():
            ...
            # 双方向の部分一致チェック
            if (
                audioset_key_lower in audioset_lower
                or audioset_lower in audioset_key_lower
            ):
